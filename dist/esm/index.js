...
import { U as Utils } from './utils-9658c8a4.js';
...
let JSX_INTERPOLATION = new Grammar({
  'embedded jsx-interpolation': {
    ...
    index(text) {
      return balanceByLexer$1(text, LEXER_BEFORE_JSX_INTERPOLATION);
    },
...
let JSX_TAG_ROOT = new Grammar({
  'jsx': {
    ...
    index: text => {
      return balanceByLexer$1(text, LEXER_TAG_ROOT);
    },
...
let JSX_TAGS = new Grammar({
  ...
  'tag tag-open': {
    ...
    index(text) {
      return balanceByLexer$1(text, LEXER_TAG_OPEN_START);
    },
...
MAIN$2.extend({
  ...
  'meta: destructuring assignment': {
    ...
    index: text => {
      ...
      let char = match[3],
          paired = pairs[char];
      let index = balance$1(text, paired, char); // Once we've balanced braces, find the next equals sign.
...
export { MAIN as Arduino, MAIN$1 as HTML, MAIN$2 as JSX, MAIN$3 as Python, MAIN$4 as Ruby, MAIN$5 as SCSS, MAIN$6 as Shell };
