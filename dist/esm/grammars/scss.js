...
const FUNCTIONS = new _daub.Grammar({
...
const INTERPOLATIONS = new _daub.Grammar({
  interpolation: {
    pattern: /(\#\{)(.*?)(})/,
    captures: {
...
const VARIABLES = new _daub.Grammar({
...
const SELECTORS = new _daub.Grammar({
...
const OPERATOR_LOGICAL = new _daub.Grammar({
  'operator operator-logical': {
    pattern: /\b(and|or|not)\b/
  }
});
const OPERATORS = new _daub.Grammar({
...
const NUMBERS = new _daub.Grammar({
  'number': {
...
const COLORS = new _daub.Grammar({
  'constant color-hex': {
...
const DIRECTIVES = new _daub.Grammar({
  'keyword directive': {
...
const PROPERTIES = new _daub.Grammar({
  'meta: property pair': {
    pattern: /([\-a-z]+)(\s*:\s*)([^;]+)(;)/,
    captures: {
      '1': 'property',
...
const INSIDE_AT_RULE_MEDIA = new _daub.Grammar({
  'support': {
...
const INSIDE_AT_RULE_IF = new _daub.Grammar({}).extend(FUNCTIONS, OPERATORS, VALUES);
const INSIDE_AT_RULE_INCLUDE = new _daub.Grammar({}).extend(PARAMETERS, VALUES, {
  'string string-unquoted': {
    pattern: /\b\w+\b/
  }
});
const INSIDE_AT_RULE_KEYFRAMES = new _daub.Grammar({
  'meta: from/to': {
...
const INSIDE_AT_RULE_SUPPORTS = new _daub.Grammar({
  'meta: property pair': {
    pattern: /([\-a-z]+)(\s*:\s*)([^;]+)(?=\)|$)/,
...
const MEDIA_AT_RULE_PROP_PAIR = new _daub.Grammar({
  'meta: property pair': {
    pattern: /([\-a-z]+)(\s*:\s*)([^;]+)(?=\)|$)/,
    captures: {
      '1': 'property',
...
const INSIDE_URL_FUNCTION = new _daub.Grammar({}).extend(STRINGS, VARIABLES, FUNCTIONS);
const AT_RULES = new _daub.Grammar({
  'meta: at-rule': {
...
const MAIN = new _daub.Grammar('scss', {});
