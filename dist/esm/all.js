...
const PARAMETERS$1 = new Grammar({
  'parameter parameter-with-default': {
    ...
    before: (r, context) => {
      r[3] = VALUES$1.parse(r[3], context);
    }
  },
  'keyword operator': {
    pattern: /\.{3}/
...
let VALUES$1 = new Grammar(_objectSpread2({
  constant: {
...
let MAIN$2 = new Grammar('javascript', {}, {
  alias: ['js']
});
...
let INSIDE_TEMPLATE_STRINGS$1 = new Grammar({
  'interpolation': {
    pattern: /(\$\{)(.*?)(\})/,
    captures: {
...
function handleJsxOrHtmlTag(tagName) {
  if (tagName.match(/^[A-Z]/)) {
    return wrap$2(tagName, 'tag tag-jsx');
  } else {
    return wrap$2(tagName, 'tag tag-html');
  }
}
...
VALUES$2.extend(ARROW_FUNCTIONS);
VALUES$2.extend(STRINGS$1);
VALUES$2.extend({
  comment: {
    pattern: /(\/\/[^\n]*\n)|(\/\*[^*]*\*+([^\/][^*]*\*+)*\/)/
  },
...
let DESTRUCTURING$1 = new Grammar({
  alias: {
    pattern: /([A-Za-z$_][$_A-Za-z0-9_]*)(\s*)(:)(\s*)(?=\w|\{|\[)/,
    captures: {
      '1': 'entity'
...
const MAIN$4 = new Grammar('python', {
...
const REGEX_INTERNALS$2 = new Grammar({
  escape: {
    pattern: /\\./
  },
...
MAIN$5.extend(VALUES$4); // These need to be lowest possible priority, so we put them in after the
// values grammar.
...
const PARAMETERS$5 = new Grammar({
  'parameter parameter-with-default': {
...
VALUES$5.extend(FUNCTIONS, STRINGS$4, COLORS, NUMBERS, DIRECTIVES, {
  'support': {
    pattern: /\b([\w-]+)\b/
  }
});
...
const INSIDE_AT_RULE_IF = new Grammar({}).extend(FUNCTIONS, OPERATORS$1, VALUES$5);
const INSIDE_AT_RULE_INCLUDE = new Grammar({}).extend(PARAMETERS$5, VALUES$5, {
...
const INSIDE_URL_FUNCTION = new Grammar({}).extend(STRINGS$4, VARIABLES, FUNCTIONS);
...
const MAIN$7 = new Grammar('shell', {
  comment: {
    pattern: /#[^\n]*(?=\n|$)/
  },
  string: {
...
export { AsyncHighlighter, Context, GRAMMARS, Grammar, Highlighter, Lexer, Utils, init$2 as init };
